			...
			if (paymentInfo) {
				...
				const updatedUser = await users.findOneAndUpdate(
					{ _id: session.client_reference_id },
					{
						$set: {
							subscription: {
								subscriptionPlanID: session.productID,
...
const cancelSubscriptionPlan = async (request, response) => {
	try {
		...
		if (authenticatingDBUser?.subscription) {
			const paymentID = authenticatingDBUser?.subscription?.paymentID;
			...
			const updatedUser = await users.findOneAndUpdate(
				{ _id: authenticatingDBUser._id },
				{
					$set: {
						...
						[`subscription.schedule`]: { start: null, end: null },
