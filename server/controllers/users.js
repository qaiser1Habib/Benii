...
const getUser = async (request, response) => {
	...
	try {
		...
		const authenticatingDBUser = await users
			...
			.populate("subscription.subscriptionPlanID")
			.populate({
				path: "therapist.clients.clientID",
				populate: {
					path: "clientAdditionalDetail.questionID",
					model: "questions",
				},
			})
			.populate({
				path: "therapist.invites",
				populate: {
					path: "clientAdditionalDetail.questionID",
					model: "questions",
				},
...
const register = async (request, response) => {
	...
	const missingFields = checkForMissingKeysInObject(payload, ["email", "password"]);
	if (missingFields) {
	...
	try {
		...
		if (dbUser) {
			...
			if (files?.length > 0) {
				for (let file of files) {
					...
					if (file.mimetype.startsWith("image")) {
						file = await optimizeAndConvertImage(file, "webp", 90);
					}
			...
			if (user.userRole === "user") {
				let therapistWithInvite = null;

				if (!payload?.therapistID) {
					therapistWithInvite = await users.findOne(
						{
							userRole: "therapist",
							"therapist.invites.email": user?.email,
						},
						{ password: 0, subscription: 0 }
					);
				}

				if (payload?.therapistID || therapistWithInvite?._id) {
					const updateTherapist = await users.findOneAndUpdate(
						{ _id: payload?.therapistID || therapistWithInvite?._id },
						{ $push: { "therapist.clients": { clientID: user._id, activityStatus: true } } },
						{ new: true }
					);

					const invitedClient = updateTherapist?.therapist?.invites.find((invite) => invite.email === user.email);

					if (invitedClient) {
						user.isVerified = { status: true, verifiedByTherapist: updateTherapist?._id, createdAt: new Date() };
						user.clientAdditionalDetail = invitedClient.clientAdditionalDetail;

						await users.findOneAndUpdate(
							{ _id: updateTherapist?._id },
							{
								$pull: { "therapist.invites": { email: user?.email } },
							},
							{ new: true }
						);
					}
	...
	try {
		...
		if (dbUser) {
			if (dbUser?.password) {
				if (dbUser?.isActive) {
					if (
						(isAdminLogin && (dbUser?.userRole === "admin" || dbUser?.userRole === "therapist")) ||
				...
				return sendJsonResponse(response, HTTP_STATUS_CODES.NOTFOUND, false, "Set Password to Continue!", null);
			}
		} else {
			return sendJsonResponse(response, HTTP_STATUS_CODES.NOTFOUND, false, "No Record Found!", null);
		}
...
const updateUser = async (request, response) => {
	try {
		...
		const isAuthenticatingDBUsersClient = authenticatingDBUser?.therapist?.clients?.some(
			(client) => client?.clientID.toString() === payload?._id
		);

		if (
			payload?._id === authenticatingUserID ||
			authenticatingDBUser?.userRole === "admin" ||
			authenticatingDBUser?.userRole === "therapist"
		) {
			if (
				payload?._id === authenticatingUserID ||
				authenticatingDBUser?.userRole === "admin" ||
				isAuthenticatingDBUsersClient
			) {
				...
				const updatedUser = await users
					...
					.populate("subscription.subscriptionPlanID")
					.populate("therapist.clients.clientID");
				...
				return sendJsonResponse(
					...
					false,
					"Access denied! User not Register Yet!",
					null
				);
			}
		} else {
		...
		if (userID === authenticatingUserID) {
			...
			if (dbUser) {
				...
				if (isPasswordMatched) {
					...
					return sendJsonResponse(response, HTTP_STATUS_CODES.CONFLICT, false, "Incorrect old password", null);
				}
			}
		} else {
			return sendJsonResponse(
		...
		if (dbUser) {
			if (dbUser?.passwordReset?.expiresAt > new Date()) {
				if (dbUser?.passwordReset?.code === decodedToken.code) {
					...
					if (updatedUser) {
						if (updatedUser.isVerified.verifiedByTherapist) {
							await users.updateOne(
								{
									_id: updatedUser.isVerified.verifiedByTherapist,
									"therapist.clients.clientID": updatedUser._id,
								},
								{
									$set: { "therapist.clients.$.activityStatus": true },
								}
							);
						}
						return sendJsonResponse(response, HTTP_STATUS_CODES.OK, true, "Password Reset::success", updatedUser);
					} else {
						return sendJsonResponse(response, HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR, false, "Password ::failure", null);
					}
